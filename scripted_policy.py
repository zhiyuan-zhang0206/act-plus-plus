...
class StirPolicy(BasePolicy):
    language_instruction :str = "put the spoon into cup and stir" # 'use the spoon to stir in the cup' # 
    def generate_trajectory(self, ts_first, random_values:list=None, hard_mode:bool=False):
        ...
        if random_values is None:
            ...
            premeet_right  = meet_xyz + np.array([np.random.uniform(0.05, 0.1), np.random.uniform(-0.03, 0.03), np.random.uniform(0.15, 0.20)])
            if hard_mode:
                premeet_left = meet_xyz + np.array([np.random.uniform(-0.06, -0.03), np.random.uniform(-0.09, +0.09), np.random.uniform(-0.03, 0.0)])
                premeet_right  = meet_xyz + np.array([np.random.uniform(0.05, 0.1), np.random.uniform(-0.09, 0.09), np.random.uniform(0.15, 0.2)])
                meet_xyz = (premeet_left + premeet_right) / 2
                meet_xyz[2] = 0.15 + np.random.uniform(-0.03, 0.03)
            random_values = {
                "meet_xyz": meet_xyz,
                # "delta_1": delta_1,
        ...
        self.left_trajectory = [
            ...
            {"t": 450,  "xyz": random_values['meet_xyz'],    "quat": left_initial_quat.elements,            "gripper": 0}, 
        ]
        ...
        self.right_trajectory = [
            ...
            {"t": 410, "xyz": random_values['meet_xyz'] + stir_deltas[2],       "quat": right_stir.elements,   "gripper":0}, # sleep
            {"t": 430, "xyz": random_values['meet_xyz'] + stir_deltas[3],       "quat": right_stir.elements,   "gripper":0}, # sleep
            {"t": 450, "xyz": random_values['meet_xyz'] + stir_deltas[0],       "quat": right_stir.elements,   "gripper":0}, # sleep
            # {"t": 430, "xyz": random_values['meet_xyz'] + stir_deltas[3],        "quat": right_stir.elements,   "gripper":0}, # sleep
...
class TransferCubePolicy(BasePolicy):
    ...
    def generate_trajectory(self, ts_first, random_values:list=None, hard_mode:bool=False):
        ...
        left_initial_loc = init_mocap_pose_left[:3]
        right_initial_loc = init_mocap_pose_right[:3]
        cube_info = np.array(ts_first.observation['env_state'])[:7]
        cube_xyz = cube_info[:3].copy()
        cube_xyz[2] = 0
        # cup_quat = cup_info[3:]
        ...
        if random_values is None:
            meet_xyz = cube_xyz.copy() # + np.random.uniform(-0.1, 0.1, 3)
            meet_xyz[2] = 0.2 # + np.random.uniform(-0.05, 0.1)
            pre_meet_left = meet_xyz + np.array([np.random.uniform(-0.03, -0.00), np.random.uniform(-0.03, +0.03), np.random.uniform(-0.03, 0.00)])
            pre_meet_right = meet_xyz + np.array([np.random.uniform(0.03, 0.06), np.random.uniform(-0.03, +0.03), np.random.uniform(0.03, 0.06)])
            if hard_mode:
        ...
        self.left_trajectory = [
            {"t": 0,    "xyz": left_initial_loc,                            "quat": left_initial_quat.elements,            "gripper": 1}, # sleep
            # {"t": 400,    "xyz": left_initial_loc,                            "quat": left_initial_quat.elements,            "gripper": 1}, # sleep
            {"t": 10,  "xyz": cube_xyz +  np.array(  [-0.03, 0.0, 0.2]),        "quat": left_initial_quat.elements,            "gripper": 1}, # sleep
            {"t": 25,  "xyz": cube_xyz + np.array(  [0.0, 0.0, 0.03]),        "quat": left_initial_quat.elements,            "gripper": 1}, # sleep
            {"t": 40,  "xyz": cube_xyz + np.array(  [0.06, 0.0, 0.03]),      "quat": left_initial_quat.elements,            "gripper": 1}, # sleep
